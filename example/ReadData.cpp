...
void InsLong() {
	...
	for (int i=0 ; i < 100; ++i)// Varchar in oracle can only be up to 4000 bytes
...
bool BulkSelVal::operator()(BoundIOs & DTL_DUMMY_PARAM_1, ExampleCharred &rowbuf)
{
...
void RandomDynamicDBView()
{
	...
	cout << random_view.size() << endl;

	try 
	{
		cout << "Items from DB:" << endl;
		// copy(random_view.begin(), random_view.end(), ostream_iterator<variant_row>(cout, "\n"));
		RandomDBView<variant_row, variant_row>::iterator it = random_view.begin(), end=random_view.end();
		while (it != end)
		{
			cout << (variant_row) *it << endl;
			++it;
		}
		cout << "\n\n";
	}
...
void RandomDBViewExample()
{
	...
	try
	{
	   ...
	   *it = row;
	   ...
	   // Under SQL server if you commit then you cannot refresh the view
	   // DBConnection::GetDefaultConnection().CommitAll();
