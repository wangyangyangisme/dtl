...
template <class T> struct dtl_set_validate
{
	...
	static bool sel_validate(BoundIOs & b, BoundIOs::UserValidate *val) 
	{
      ...
      {
         const typename dtl_set<T>::sql_type* s = std::find(&dtl_set<T>::sql_values[0],&dtl_set<T>::sql_values[dtl_set<T>::num], tok);
...
template <class T> BoundIO &dtl_set_bind(BoundIO & b, dtl_set<T> & x) 
{
   ...
   ptrdiff_t offset = reinterpret_cast<BYTE*>(&x) - b.GetBoundIOsPtr()->GetWorkingAddr();
